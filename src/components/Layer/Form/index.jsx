...
class LayerForm extends React.Component {
  ...
  componentWillMount() {
    if (this.state.dataset && this.state.layer) {
      ...
      xmlhttp.open('GET', `http://api.resourcewatch.org/v1/dataset/${this.state.dataset}/layer/${this.state.layer}?cache=${Date.now()}`);
    ...
    setTimeout(() => {
      ...
      if (valid) {
        if (this.state.step === this.state.stepLength && !this.state.submitting) {
          ...
          xmlhttp.open(xmlhttpOptions.type, `http://api.resourcewatch.org/v1/dataset/${this.state.dataset}/layer/${this.state.layer || ''}`);
